...
Int8EntropyCalibrator2::Int8EntropyCalibrator2(
  ...
  if (!doesFileExist(calibDataDirPath)) {
    throw std::runtime_error(
        "Error, directory at provided path does not exist: " +
        calibDataDirPath);
  }
  ...
  if (m_imgPaths.size() < static_cast<size_t>(batchSize)) {
    throw std::runtime_error(
        "There are fewer calibration images than the specified batch size!");
  }
...
bool Int8EntropyCalibrator2::getBatch(void **bindings, const char **names,
  ...
  for (int i = m_imgIdx; i < m_imgIdx + m_batchSize; i++) {
    ...
    if (cpuImg.empty()) {
      std::cout << "Fatal error: Unable to read image at path: "
  ...
  auto mfloat = Engine<float>::blobFromGpuMats(inputImgs, m_subVals, m_divVals,
                                               m_normalize);
...
void Int8EntropyCalibrator2::writeCalibrationCache(
  ...
  std::cout << "Writing calib cache: " << m_calibTableName
