...
inline bool tryGetNextArgument(int argc, char *argv[], int &currentIndex,
                               std::string &value, std::string flag,
                               bool printErrors = true) {
  if (currentIndex + 1 >= argc) {
    if (printErrors)
      std::cout << "Error: No arguments provided for flag '" << flag << "'"
                << std::endl;
    return false;
  }
...
inline bool parseArguments(int argc, char *argv[],
    ...
    if (argument.substr(0, 2) == "--") {
      ...
      if (flag == "onnx_model") {
        ...
        if (!Util::doesFileExist(nextArgument)) {
          std::cout << "Error: Unable to find model at path '" << nextArgument
  ...
  if (arguments.onnxModelPath.empty() && arguments.trtModelPath.empty()) {
    std::cout << "Error: Must specify either 'onnx_model' or 'trt_model'"
              << std::endl;
    return false;
  }
