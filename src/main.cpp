...
int main(int argc, char *argv[]) {
  ...
  if (!arguments.onnxModelPath.empty()) {
    ...
    bool succ =
        engine.loadNetwork(arguments.trtModelPath, subVals, divVals, normalize);
    if (!succ) {
  ...
  std::cout << "Warmup done. Running benchmarks (" << numIterations
            << " iterations)...\n"
            << std::endl;
  ...
  auto avgElapsedTimeMs =
      totalElapsedTimeMs / numIterations / static_cast<float>(inputs[0].size());
