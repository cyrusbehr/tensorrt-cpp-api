...
public:
  Int8EntropyCalibrator2(int32_t batchSize, int32_t inputW, int32_t inputH,
...
public:
  ...
  [[nodiscard]] const std::vector<nvinfer1::Dims3> &getInputDims() const {
    return m_inputDims;
  };
  ...
  blobFromGpuMats(const std::vector<cv::cuda::GpuMat> &batchInput,
...
bool Engine<T>::buildLoadNetwork(std::string onnxModelPath,
  ...
  std::cout << "Searching for engine file with name: " << engineName
            << std::endl;
  ...
  if (Util::doesFileExist(engineName)) {
    ...
    std::cout << "Engine not found, generating. This could take a while..."
              << std::endl;
...
bool Engine<T>::loadNetwork(std::string trtModelPath,
  ...
  if (!Util::doesFileExist(trtModelPath)) {
    std::cout << "Error, unable to read TensorRT model at path: " + trtModelPath
              << std::endl;
    return false;
  } else {
    std::cout << "Loading TensorRT engine file at path: " << trtModelPath
              << std::endl;
  }
  ...
  for (int i = 0; i < m_engine->getNbIOTensors(); ++i) {
    ...
    if (tensorType == nvinfer1::TensorIOMode::kINPUT) {
      ...
      Util::checkCudaErrorCode(cudaMallocAsync(
          &m_buffers[i], outputLength * m_options.maxBatchSize * sizeof(T),
          stream));
    } else {
      throw std::runtime_error(
...
bool Engine<T>::runInference(
  ...
  if (inputs[0].size() > static_cast<size_t>(m_options.maxBatchSize)) {
    ...
    std::cout << "Model max batch size: " << m_options.maxBatchSize
              << std::endl;
    std::cout << "Batch size provided to call to runInference: "
  ...
  if (m_inputBatchSize != -1 &&
    ...
    std::cout << "===== Error =====" << std::endl;
    std::cout << "The batch size is different from what the model expects!"
              << std::endl;
  ...
  for (size_t i = 1; i < inputs.size(); ++i) {
    if (inputs[i].size() != static_cast<size_t>(batchSize)) {
      std::cout << "===== Error =====" << std::endl;
      std::cout << "The batch size needs to be constant for all inputs!"
                << std::endl;
      return false;
    }
  }
  ...
  for (size_t i = 0; i < numInputs; ++i) {
    ...
    if (input.channels() != dims.d[0] || input.rows != dims.d[1] ||
      ...
      std::cout << "Expected: (" << dims.d[0] << ", " << dims.d[1] << ", "
                << dims.d[2] << ")" << std::endl;
      std::cout << "Got: (" << input.channels() << ", " << input.rows << ", "
                << input.cols << ")" << std::endl;
      std::cout << "Ensure you resize your input image to the correct size"
                << std::endl;
      return false;
    }
    ...
    for (int32_t outputBinding = numInputs;
      ...
      Util::checkCudaErrorCode(
          cudaMemcpyAsync(output.data(),
                          ...
                          outputLength * sizeof(T), cudaMemcpyDeviceToHost,
                          inferenceCudaStream));
...
Engine<T>::blobFromGpuMats(const std::vector<cv::cuda::GpuMat> &batchInput,
  ...
  cv::cuda::GpuMat gpu_dst(
      1, batchInput[0].rows * batchInput[0].cols * batchInput.size(), CV_8UC3);
